...
def bisplrep_f2py(x, y, z, kx=3, ky=3, s=0):
    """Direct f2py wrapper call mimicking bisplrep."""
    m = len(x)
    ...
    wrk = np.zeros(lwrk1, dtype=np.float64)
    kwrk = m + (nxest - 2*kx - 1)*(nyest - 2*ky - 1)
    iwrk = np.zeros(kwrk, dtype=np.int32)
    
    # Call the Fortran routine directly (positional arguments)
    output = _fitpack._surfit(
        x, y, z, w, xb, xe, yb, ye, kx, ky,
    ...
    if len(output) == 6:
        ...
        fp = output[3]
        ier = output[4]
        msg = "" if len(output) < 6 else output[5]
    
    if ier > 0:
        raise ValueError(f"Error in surfit: {msg}")
    
    # Find actual knot counts
    nx = len([t for t in tx if xb <= t <= xe])
    ny = len([t for t in ty if yb <= t <= ye])
