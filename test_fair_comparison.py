...
start = time.perf_counter()
for _ in range(1000):
    result = fast_bisplev(x_test, y_test, tck_fast)
...
# FastSpline - can handle arrays directly
start = time.perf_counter()
result_fast = fast_bisplev(x_scatter, y_scatter, tck_fast)
...
print(f"  Max difference (grid): {np.max(np.abs(result_scipy - result_fast)):.2e}")
print(f"  Max difference (scattered): {np.max(np.abs(result_scipy - result_fast)):.2e}")