#!/usr/bin/env python3
"""
Test script for Sergei's 2D spline cfunc implementation - Fixed version
"""

import numpy as np
import matplotlib.pyplot as plt
import ctypes
from sergei_splines_cfunc_2d_fixed import get_cfunc_addresses
...
def setup_ctypes_signatures():
    ...
    construct_2d = ctypes.CFUNCTYPE(
        ...
        ctypes.POINTER(ctypes.c_double),  # workspace
...
def test_2d_spline(order=(3, 3), periodic=(False, False)):
    ...
    # Allocate workspace for construction
    # Need space for: max((o2+1)*n2 + 2*n2, (o1+1)*n1 + 2*n1)
    workspace_size = max((order[1]+1)*n2 + 2*n2, (order[0]+1)*n1 + 2*n1)
    workspace_c = (ctypes.c_double * workspace_size)()
    
    # Construct spline
    print(f"Constructing 2D spline with order={order}, periodic={periodic}")
    construct_2d(x_min_c, x_max_c, z_c, num_points_c, order_c, periodic_c, coeff_c, workspace_c)
    ...
    # Workspace for evaluation (max_order + 1)
    eval_workspace_size = max(order[0], order[1]) + 1
    eval_workspace_c = (ctypes.c_double * eval_workspace_size)()
    ...
    for i in range(n_eval):
        for j in range(n_eval):
            ...
            evaluate_2d(order_c, num_points_c, periodic_c, x_min_c, h_step_c, 
                       coeff_c, x_point, y_out, eval_workspace_c)
            Z_eval[i, j] = y_out[0]
    ...
    plt.tight_layout()
    plt.savefig(f'sergei_2d_spline_fixed_order_{order[0]}_{order[1]}_periodic_{periodic[0]}_{periodic[1]}.png')
    plt.show()
...
def test_1d_comparison():
    """Test 1D spline to verify basic functionality"""
    from sergei_splines_cfunc_2d_fixed import construct_spline_1d_wrapper, evaluate_spline_1d_wrapper
    ...
    # Construct spline
    spline = construct_spline_1d_wrapper(x_min, x_max, y_data, order=3, periodic=False)
    
    # Evaluate on fine grid
    x_eval = np.linspace(x_min, x_max, 200)
    y_eval = evaluate_spline_1d_wrapper(spline, x_eval)
    
    # Plot
    plt.figure(figsize=(10, 6))
    plt.plot(x_data, y_data, 'ro', label='Data points')
    plt.plot(x_eval, y_eval, 'b-', label='Spline interpolation')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title('1D Spline Test')
    plt.legend()
    plt.grid(True)
    plt.savefig('sergei_1d_spline_test.png')
    plt.show()
    
    # Check interpolation error at data points
    y_at_data = evaluate_spline_1d_wrapper(spline, x_data)
    max_error = np.max(np.abs(y_at_data - y_data))
    print(f"1D interpolation error: {max_error:.2e}")


if __name__ == "__main__":
    print("Testing Sergei's spline implementation (Fixed version)\n")
    
    # First test 1D to ensure basics work
    print("Testing 1D spline functionality...")
    test_1d_comparison()
    
    print("\n\nTesting 2D spline functionality...")
